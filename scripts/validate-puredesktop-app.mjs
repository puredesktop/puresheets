...
function validateManifest(manifest) {
  // Current suite AppManifestSchema is strict and has no schemaVersion key.
  if ('schemaVersion' in manifest) errors.push('schemaVersion is not supported by the current suite manifest schema.')
...
async function validateAgentSurface(manifest, distPath) {
  ...
  } else if (manifest.app?.usePureDesktopAiPanel !== undefined &&
    typeof manifest.app.usePureDesktopAiPanel !== 'boolean') {
    errors.push('app.usePureDesktopAiPanel must be a boolean when supplied.')
  }
